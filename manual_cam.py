...
# try:
    # while driver.step() != -1:
        ...
        # if step_counter >= steps_per_frame:
            ...
            # img_name = f"img_{frame_idx:06d}.png"  
...
# RUN_NAME    = time.strftime("test_clear_%Y%m%d_%H%M%S")  # clearly labeled as clear test set
# DATA_ROOT   = "dataset"
# DATA_DIR    = os.path.join(DATA_ROOT, RUN_NAME)
# IMG_DIR     = os.path.join(DATA_DIR, "images")
# CSV_PATH    = os.path.join(DATA_DIR, "labels.csv")
...
# csv_file = open(CSV_PATH, "w", newline="")
# csvw = csv.writer(csv_file)
# csvw.writerow(["image", "steer_rad", "steer_norm"])  # no 'condition' column

# print(f"manual_cam_logger_test running — logging to: {DATA_DIR}")
# width, height = cam.getWidth(), cam.getHeight()
# print(f"camera enabled: {width}x{height} @ ~{1000.0/timestep_ms:.1f} Hz sim step; capturing at {TARGET_HZ} Hz")
...
# RUN_NAME    = time.strftime("test_fog_%Y%m%d_%H%M%S")  # fog test dataset name
# DATA_ROOT   = "dataset"
# DATA_DIR    = os.path.join(DATA_ROOT, RUN_NAME)
# IMG_DIR     = os.path.join(DATA_DIR, "images")
# CSV_PATH    = os.path.join(DATA_DIR, "labels.csv")
# TARGET_HZ   = 10
# SAVE_QUALITY = 100  # PNG quality setting (ignored for PNG)
...
# QUIT_KEYS = {27, ord('Q'), ord('q')}  # Esc/Q/q to quit
...
# csv_file = open(CSV_PATH, "w", newline="")
# csvw = csv.writer(csv_file)
# csvw.writerow(["image", "steer_rad", "steer_norm"])  # no condition column — just fog dataset name

# print(f"✅ manual_cam_logger_test_fog running — logging to: {DATA_DIR}")
# width, height = cam.getWidth(), cam.getHeight()
# print(f"camera enabled: {width}x{height} @ ~{1000.0/timestep_ms:.1f} Hz sim step; capturing at {TARGET_HZ} Hz")
...
# ========= CONFIG =========
RUN_NAME    = time.strftime("test_storm_%Y%m%d_%H%M%S")  # storm test dataset name
DATA_ROOT   = "dataset"
DATA_DIR    = os.path.join(DATA_ROOT, RUN_NAME)
IMG_DIR     = os.path.join(DATA_DIR, "images")
CSV_PATH    = os.path.join(DATA_DIR, "labels.csv")
...
csv_file = open(CSV_PATH, "w", newline="")
csvw = csv.writer(csv_file)
csvw.writerow(["image", "steer_rad", "steer_norm"])  # no condition column — just storm dataset name

print(f"🌩️ manual_cam_logger_test_storm running — logging to: {DATA_DIR}")
width, height = cam.getWidth(), cam.getHeight()
print(f"camera enabled: {width}x{height} @ ~{1000.0/timestep_ms:.1f} Hz sim step; capturing at {TARGET_HZ} Hz")

# 10 Hz capture scheduler
steps_per_sec   = 1000.0 / timestep_ms
steps_per_frame = max(1, int(round(steps_per_sec / TARGET_HZ)))
step_counter    = 0
frame_idx       = 0
...
try:
    while driver.step() != -1:
        ...
        if step_counter >= steps_per_frame:
            ...
            csvw.writerow([img_name, f"{steer:.4f}", f"{steer/STEER_MAX:.4f}"])
            csv_file.flush()
            frame_idx += 1
    ...
    csv_file.close()
    print(f"✅ Logged {frame_idx} frames to {CSV_PATH}")
